import { useState, useEffect, useCallback, useMemo, useImperativeHandle, use } from 'react';
		...
		const attemptAutoLogin = async () => {
			...
			if (isTokenValid(accessToken)) {
				try {
					...
					setTokenStorageValue(auth.tokens.accessToken);
					setGlobalHeaders({ Authorization: `Bearer ${auth.tokens.accessToken}` });
					return auth.user;
				} catch (error) {
	...
	const signIn: JwtAuthContextType['signIn'] = useCallback(
		async (credentials) => {
			try {
				...
				setTokenStorageValue(session.tokens.accessToken);
				setGlobalHeaders({ Authorization: `Bearer ${session.tokens.accessToken}` });
				const formatSesstion = {
					user: session.user,
					access_token: session.tokens.accessToken
				}
				return formatSesstion;
			} catch (error) {
				if (error instanceof HTTPError) {
