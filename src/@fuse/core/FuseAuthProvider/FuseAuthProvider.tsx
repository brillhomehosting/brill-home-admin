...
import FuseAuthContext, { AuthState, initialAuthState } from './FuseAuthContext';
import { FuseAuthProviderMethods, FuseAuthProviderState, FuseAuthProviderType } from './types/FuseAuthTypes';
	...
	const handleAuthStateChange = useCallback(
		(providerAuthState: FuseAuthProviderState, name: string) => {
			...
			setAuthState((prev) => {
				...
				// For token-based providers the `user` may be null, so rely on `isAuthenticated`
				if (!prev.isAuthenticated && providerAuthState.isAuthenticated) {
